...
try:
    ...
    print("Warning: reportlab, matplotlib, or shap not found. PDF export will be disabled.")
...
class ActivityLog(Base):
    ...
    action_type = Column(String, nullable=False, index=True) # e.g., 'LOGIN', 'LOGOUT', 'ALERT_UPDATE', 'MODEL_RELOAD'
...
async def login_admin(request: Request, username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    log_activity("SYSTEM_ADMIN", 'LOGIN_FAILURE', {'ip': request.client.host, 'attempted_user': username})
